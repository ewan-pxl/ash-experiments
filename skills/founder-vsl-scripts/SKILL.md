---
name: founder-vsl-scripts
description: Produces founder-led video sales letter (VSL) scripts in a multi-theme, multi-version structure designed for confessional, founder-to-camera videos used in paid social, YouTube pre-roll, and homepage hero placements. Use this skill any time the user asks for VSL scripts, founder videos, talking-head ads, founder testimonial scripts, "Oliver-style" or "founder reads-to-camera" content, or asks to turn brand context into video scripts. Also trigger when the user references the Void Health TRT founder transcript style, asks for confessional/off-the-cuff video scripts, requests "themes and versions" of video scripts, or wants scripts paired with pause direction and scene direction. The skill produces three themes × two versions (polished + conversational) with pause notation, performance direction, and a structured Notion deliverable.
---

# Founder VSL Scripts

A workflow for producing founder-led video sales letter scripts that don't sound like ads. Built around the Void Health TRT founder transcript style — confessional, slow, the product mentioned almost in passing — paired with a more polished script-led version for variety in paid testing.
...
- **Version A · Polished (script-led).** Tighter cadence, declarative tone. Read aloud from teleprompter. Best for paid social where pace matters. ~190–210 words.
- **Version B · Conversational (off-the-cuff).** Confessional tone, modelled on the Void TRT founder transcript. Should feel unrehearsed. Best for YouTube pre-roll and longer-form environments. ~210–225 words.
...
If the user has Notion connected, build the document directly in their workspace. Use the structure in `references/notion-template.md`. If Notion is not connected, produce the content as Markdown for the user to paste in.
...
## Deliverable structure (Notion document)

The document should follow this exact structure. See `references/notion-template.md` for full formatting with colours, callouts, and toggles.

```
# Founder VSL Scripts — [Founder Name]
*Brief intro line about runtime, placements, and intended use.*

## How to use this document
- Explanation of A vs B versions
- IMPORTANT callout for the founder: rehearse 5–10x then go natural, switch things up on the fly

## Pause notation key
- [beat] / [pause] / [long pause] / italics
...
## Version A · Polished
[script with pause notation]
*~word count · ~runtime*
<details>Pause logic for Version A</details>

## Version B · Conversational
[script with pause notation]
*~word count · ~runtime*
<details>Pause logic for Version B</details>
...
# Scene direction
[tight version: framing, lighting, set, wardrobe, performance]
...
[which scripts to test against each other and why]
```
